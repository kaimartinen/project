...
///////////////////////////////////////////////////////////////////////////////
double Line::ranges_max()
{
  double max = 0;
  for(std::vector<double>::const_iterator cit = r_data_.ranges.begin(); cit != r_data_.ranges.end(); ++cit)
  {
    if (max < *cit) {
      max = *cit;
    }
    if (max == r_data_.max_range) {
      break;
    }
  }
  return max;
...
void  Line::setEnd(boost::array<double, 2> end)
{
  end_ = end;
}
...
void  Line::setStart(boost::array<double, 2> start)
{
  start_ = start;
}
