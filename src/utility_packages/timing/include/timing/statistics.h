...
{
    if(strlen(_name) == 0)
        printf("Mean %.3f %s Std: %.3f %s Total: %.3f %s (Num: %.0f)\n", getMean()*factor, unit, getStandardDeviation()*factor, unit, 
                getMean() * _numMeasurements * factor, unit, _numMeasurements);
    else
        printf("%s: Mean %.3f %s Std: %.3f %s Total: %.3f %s (Num: %.0f)\n", _name, getMean()*factor, unit, getStandardDeviation()*factor, unit,
                getMean() * _numMeasurements * factor, unit, _numMeasurements);
}
